...
ApplicationWindow {
    ...
    // ================== КОМПОНЕНТ: ВЕКТОРНАЯ ИКОНКА ==================
        component VectorIcon: Canvas {
            ...
            onPaint: {
                ...
                if (name === "dashboard") {
                    // 4 квадрата
                    ctx.strokeRect(3, 3, 8, 8);
                    ctx.strokeRect(13, 3, 8, 8);
                    ctx.strokeRect(3, 13, 8, 8);
                    ctx.strokeRect(13, 13, 8, 8);
                    // Зальем один для акцента (опционально, можно убрать fillRect)
                    if (color == theme.accentColor) ctx.fillRect(3, 3, 8, 8);
                }
                else if (name === "list") {
                    // Список
                    ctx.strokeRect(4, 3, 16, 18);
                    ctx.beginPath();
                    ctx.moveTo(8, 8); ctx.lineTo(16, 8);
                    ctx.moveTo(8, 12); ctx.lineTo(16, 12);
                    ctx.moveTo(8, 16); ctx.lineTo(13, 16);
                    ctx.stroke();
                }
                else if (name === "wallet") {
                    // ДОХОДЫ (Купюра) - Исправлено (убрал roundRect)
                    ctx.beginPath();
                    ctx.rect(2, 6, 20, 12); // Контур
                    ctx.stroke();
                    // Круг по центру
                    ctx.beginPath();
                    ctx.arc(12, 12, 3, 0, Math.PI*2);
                    ctx.stroke();
                    // Маленькие кружки по бокам
                    ctx.beginPath(); ctx.arc(5, 12, 1, 0, Math.PI*2); ctx.fill();
                    ctx.beginPath(); ctx.arc(19, 12, 1, 0, Math.PI*2); ctx.fill();
                }
                else if (name === "chart") {
                    // Аналитика
                    ctx.beginPath();
                    ctx.moveTo(2, 20); ctx.lineTo(22, 20); // Ось
                    ctx.moveTo(6, 20); ctx.lineTo(6, 12); // Столбик 1
                    ctx.moveTo(12, 20); ctx.lineTo(12, 6);  // Столбик 2
                    ctx.moveTo(18, 20); ctx.lineTo(18, 14); // Столбик 3
                    ctx.stroke();
                }
                else if (name === "target") {
                    // Цели
                    ...
                    // Лампочка
                    ctx.beginPath(); ctx.arc(12, 9, 5, Math.PI, 0); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(7, 9); ctx.lineTo(10, 15); ctx.lineTo(14, 15); ctx.lineTo(17, 9); ctx.stroke();
                    ctx.fillRect(10, 15, 4, 3);
                }
                else if (name === "exit") {
                    // Выход
                    ctx.beginPath(); ctx.moveTo(10, 5); ctx.lineTo(19, 5); ctx.lineTo(19, 19); ctx.lineTo(10, 19); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(10, 12); ctx.lineTo(2, 12); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(6, 9); ctx.lineTo(2, 12); ctx.lineTo(6, 15); ctx.stroke();
                }
                // Стрелки для дашборда
                else if (name === "arrow_up") {
                    ctx.beginPath(); ctx.moveTo(12, 17); ctx.lineTo(12, 7); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(7, 12); ctx.lineTo(12, 7); ctx.lineTo(17, 12); ctx.stroke();
                }
                else if (name === "arrow_down") {
                    ctx.beginPath(); ctx.moveTo(12, 7); ctx.lineTo(12, 17); ctx.stroke();
                    ctx.beginPath(); ctx.moveTo(7, 12); ctx.lineTo(12, 17); ctx.lineTo(17, 12); ctx.stroke();
                }
                else if (name === "briefcase") {
                    ctx.strokeRect(4, 7, 16, 12);
                    ctx.beginPath(); ctx.moveTo(9, 7); ctx.lineTo(9, 4); ctx.lineTo(15, 4); ctx.lineTo(15, 7); ctx.stroke();
                }
            }
        }
    ...
    component SidebarButton: Rectangle {
        ...
        property string iconName // Имя иконки вместо символа
        property bool isActive: false
        signal clicked()
        Layout.fillWidth: true
        Layout.preferredHeight: 46
        color: isActive ? "#1e293b" : "transparent" // Более мягкое выделение
        radius: 8

        // Полоска активности слева
        Rectangle {
        ...
        RowLayout {
            ...
            VectorIcon { name: iconName; color: isActive ? theme.accentColor : theme.subTextColor }
            Text {
                text: parent.parent.text; color: isActive ? "white" : theme.subTextColor
                font.pixelSize: 14; font.bold: isActive; Layout.fillWidth: true
            }
        }
    ...
    component DesktopTextField: TextField {
        ...
        font.pixelSize: 14; color: theme.textColor; placeholderTextColor: theme.subTextColor
        selectByMouse: true; verticalAlignment: TextInput.AlignVCenter
    ...
    component PrimaryButton: Button {
        id: btn
        Layout.preferredHeight: 38; Layout.preferredWidth: 140
    ...
    component StatCard: Rectangle {
        ...
        color: theme.cardColor; radius: 12; border.color: theme.borderColor; border.width: 1
        RowLayout {
            anchors.fill: parent; anchors.margins: 20; spacing: 15
            Rectangle {
                Layout.preferredWidth: 48; Layout.preferredHeight: 48; radius: 12
                color: Qt.rgba(iconColor.r, iconColor.g, iconColor.b, 0.15) // Полупрозрачный фон
                VectorIcon { anchors.centerIn: parent; name: iconName; color: iconColor }
            }
            ColumnLayout {
                spacing: 4
                Text { text: title; color: theme.subTextColor; font.pixelSize: 13; font.weight: Font.Medium }
                Text { text: value; color: "white"; font.pixelSize: 22; font.bold: true }
            }
        }
    }
    ...
    RowLayout {
        ...
        // САЙДБАР
                Rectangle {
                    ...
                    color: theme.sidebarColor; border.color: theme.borderColor; border.width: 1
                    ColumnLayout {
                        ...
                        // ВОТ ЭТА СТРОКА ОТВЕЧАЕТ ЗА ИКОНКУ ДОХОДОВ:
            ...
            StackLayout {
                ...
                ColumnLayout {
                    ...
                    RowLayout {
                        ...
                        StatCard { title: "Доход"; value: "+" + mainWindow.budget.toFixed(2); iconName: "arrow_up"; iconColor: theme.incomeColor }
                        StatCard { title: "Расход"; value: "−" + mainWindow.spent.toFixed(2); iconName: "arrow_down"; iconColor: theme.expenseColor }
                    }
                    Rectangle {
                        ...
                        ColumnLayout {
                            ...
                            Rectangle {
                                ...
                                RowLayout {
                                    ...
                                    Text { text: "Категория"; color: theme.subTextColor; font.bold: true; Layout.fillWidth: true; font.pixelSize: 12 }
                ...
                ColumnLayout {
                    spacing: 20
                    Text { text: "Аналитика расходов"; font.pixelSize: 24; font.bold: true; color: "white" }
                    RowLayout {
                        ...
                        Rectangle {
                            ...
                            ColumnLayout {
                                ...
                                Canvas {
                                    width: 220; height: 220; antialiasing: true
                                    property var dataModel: mainWindow.analyticsData
                                    onDataModelChanged: requestPaint()
                                    onPaint: {
                                        var ctx = getContext("2d"); ctx.reset(); var cx = width/2; var cy = height/2; var r = 80; var startAngle = -Math.PI/2; ctx.lineWidth = 30;
                                        if (dataModel && dataModel.length > 0) {
                                            for (var i = 0; i < dataModel.length; i++) {
                                                var sweep = dataModel[i].percent * 2 * Math.PI;
                                                ctx.beginPath(); ctx.arc(cx, cy, r, startAngle, startAngle + sweep, false);
                                                ctx.strokeStyle = catColors[i % catColors.length]; ctx.stroke(); startAngle += sweep;
                                            }
                                        } else { ctx.beginPath(); ctx.arc(cx, cy, r, 0, 2*Math.PI); ctx.strokeStyle = theme.borderColor; ctx.stroke(); }
                                    }
                                }
                        ...
                        Rectangle {
                            ...
                            Canvas {
                                id: barChart; anchors.fill: parent; anchors.margins: 40; antialiasing: true
                                property var dataModel: mainWindow.analyticsData
                                onDataModelChanged: requestPaint()
                                onPaint: {
                                    var ctx = getContext("2d"); ctx.reset(); var w = width; var h = height;
                                    ctx.beginPath(); ctx.strokeStyle = theme.subTextColor; ctx.lineWidth = 2;
                                    ctx.moveTo(0, 0); ctx.lineTo(0, h); ctx.lineTo(w, h); ctx.stroke();
                                    if (!dataModel || dataModel.length === 0) return;
                                    var maxVal = 0; for(var i=0; i<dataModel.length; i++) if(dataModel[i].amount > maxVal) maxVal = dataModel[i].amount;
                                    if(maxVal === 0) maxVal = 1;
                                    var barWidth = (w / dataModel.length) * 0.5; var gap = (w / dataModel.length) * 0.5; var x = gap/2;
                                    for(var i=0; i<dataModel.length; i++) {
                                        var barH = (dataModel[i].amount / maxVal) * (h - 30);
                                        ctx.fillStyle = catColors[i % catColors.length];
                                        ctx.fillRect(x, h - barH, barWidth, barH);
                                        ctx.fillStyle = theme.subTextColor; ctx.font = "bold 11px sans-serif";
                                        ctx.fillText(dataModel[i].category.substring(0,3), x, h+15);
                                        x += barWidth + gap;
                                    }
                                }
                            }
                    ...
                    Rectangle {
                        ...
                        ListView {
                            ...
                            delegate: RowLayout {
                                Rectangle { width: 12; height: 12; radius: 6; color: catColors[index % catColors.length] }
                                Column { Text { text: modelData.category; color: theme.textColor; font.bold: true; font.pixelSize: 13 } Text { text: modelData.amount.toFixed(2) + " Br (" + (modelData.percent*100).toFixed(1) + "%)"; color: theme.subTextColor; font.pixelSize: 11 } }
                            }
                        }
                    }
                }
                ...
                ColumnLayout {
                    spacing: 20
                    Text { text: "Умный помощник"; font.pixelSize: 24; font.bold: true; color: "white" }
